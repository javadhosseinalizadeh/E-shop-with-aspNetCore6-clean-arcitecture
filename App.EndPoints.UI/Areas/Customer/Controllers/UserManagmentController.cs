...
namespace App.EndPoints.UI.Areas.Customer.Controllers
{
    ...
    public class UserManagmentController : Controller
    {
        ...
        {
            List<CustomerMngmntViewModel> model;
            if (string.IsNullOrEmpty(name))
            {
                model = await _userManager.Users.Select(x => new CustomerMngmntViewModel
                {
                    Id = x.Id,
                    Name = x.UserName,
                    Email = x.Email,
            ...
            else
            {
                model = await _userManager.Users
                    ...
                    .Select(x => new CustomerMngmntViewModel
                    {
                        Id = x.Id,
                        Name = x.UserName,
                        Email = x.Email,
        ...
        {
            var user = await _userManager.Users.FirstAsync(x => x.Id == userId);
            var model = new CustomerMngmntViewModel
            {
                Id = user.Id,
                Name = user.UserName,
                Email = user.Email,
        ...
        [HttpPost]
        public async Task<IActionResult> Update(CustomerMngmntViewModel model)
        {
